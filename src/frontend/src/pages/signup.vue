    ...
    <v-row align="center" justify="center">
      ...
      <v-col lg="4" xl="4" cols="12">
        ...
        <v-form ref="form" validate-on="submit lazy" @submit.prevent="submit">
          ...
          <v-btn block color="primary" :loading="loading" type="submit">Sign up</v-btn>
        </v-form>
        <v-divider class="my-5"></v-divider>
...
const checkApi = (user, email, pass) => {
  return new Promise(resolve => {
    ...
    if (!user) {
      errors.push({ key: 'user', value: 'Username cannot be blank.' })
    }
    ...
    if (errors.length > 0)
      ...
      return appStore.register({username: user, email: email, password: pass})
    }
  });
};

const submit = async () => {
  const results = await checkApi(username.value, email.value, password.value);
