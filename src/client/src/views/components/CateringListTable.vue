  ...
  <div class="card">
    ...
    <div class="card-body px-0 pt-0 pb-2">
      <div class="table-responsive p-0">
        <table class="table align-items-center mb-0" v-if="list.length > 0">
          <thead>
          <tr>
            <th
        ...
        <div v-else class="card-header pb-0 d-flex justify-content-between align-items-center">
          You need to create a list first!
        </div>
        <div class="card-header pb-0 d-flex justify-content-between align-items-center">
          <h6>Other Lists (Invites)</h6>
        </div>
        <table class="table align-items-center mb-0" v-if="invList.length > 0">
          <thead>
          <tr>
            <th
                class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7"
          ...
          <tbody>
          <tr v-for="(value, index) in invList" :key="index">
            <td>
              <div class="d-flex px-2 py-1">
                <div>
                  <img :src="`/api/v1/image/list/${value.id}`" class="avatar avatar-sm me-3" alt="user2" />
        ...
        <div v-else class="card-header pb-0 d-flex justify-content-between align-items-center">
          You need to be invited to a list first!
        </div>
      </div>
    </div>
    <edit-catering-list
