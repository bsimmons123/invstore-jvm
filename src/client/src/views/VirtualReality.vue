...
const argonStore = store.state.argon;

const sidebarMinimize = () => store.commit("argon/sidebarMinimize");
...
onBeforeMount(() => {
  argonStore.layout = "vr";
  argonStore.showNavbar = false;
  argonStore.showSidenav = false;
  argonStore.showFooter = false;
  body.classList.add("virtual-reality");
  argonStore.isTransparent = "bg-white";
});
onBeforeUnmount(() => {
  argonStore.layout = "default";
  argonStore.showNavbar = true;
  argonStore.showSidenav = true;
  argonStore.showFooter = true;
  body.classList.remove("virtual-reality");

  if (argonStore.isPinned === false) {
    const sidenav_show = document.querySelector(".g-sidenav-show");
    sidenav_show.classList.remove("g-sidenav-hidden");
    sidenav_show.classList.add("g-sidenav-pinned");
    argonStore.isPinned = true;
  }
  argonStore.isTransparent = "bg-transparent";
});
</script>
<template>
  <div class="mt-3">
    <navbar
      ...
      :class="`${argonStore.isNavFixed ? argonStore.navbarFixed_class : ''} ${
        argonStore.isNavFixed ? 'bg-white' : 'bg-success'
      }`"
    />
  </div>
  <div
