...
import StoreIndex from "@/store/profile/_StoreIndex";
import {StoreActions} from "@/store/profile/actions";
...
const user = computed(() => profileState.user);
const loadingUser = computed(() => profileState.loadingUser);

const getUserProfile = () => store.dispatch(`${StoreIndex.storeName}/${StoreActions.getUser}`);

onMounted(() => {
  argonState.isAbsolute = true;
  setNavPills();
  setTooltip();
  getUserProfile()
});
onBeforeMount(() => {
  argonState.imageLayout = "profile-overview";
...
onBeforeUnmount(() => {
  argonState.isAbsolute = false;
  argonState.imageLayout = "default";
...
<!--              </div>-->
<!--            </div>-->
            <div class="col-auto my-auto" v-if="!loadingUser">
              <div class="h-100">
